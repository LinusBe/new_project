...
def optimize_dataframe_types(df):
    """
    Attempts to intelligently convert column data types (text to numbers/date).
    Assumes that the data was initially loaded robustly as text.
    """
    ...
    # Entferne gängige Währungssymbole und Whitespace
                ...
                logger.debug(f"Numeric conversion for column {col} failed: {e}")
            ...
            try:
               ...
               logger.debug(f"Date conversion for column {col} failed: {e}")
    return df

def read_csv_robust(file_content_bytes):
    """Reads CSVs with automatic encoding and separator sniffing."""
    detection = chardet.detect(file_content_bytes[:307200]) # Analysis range increased (300KB)
    ...
    raise ValueError("Could not reliably determine the CSV format (Encoding/Separator).")
...
def download_file(request, file_id):
    ...
    try:
        ...
        raise Http404("File not present in storage.")
    except Exception as e:
...
def analyze_file(request, file_id):
    ...
    if pa is None:
        messages.error(request, "Analysis function disabled. The server dependency PyArrow is missing.")
        return redirect('dashboard')
    ...
    if not data_file.is_analyzable():
        messages.warning(request, f"This file type cannot be analyzed online.")
        return redirect('dashboard')
...
def render_analysis_template(request, data_file, selected_sheet):
    """Renders the HTML page, including the Excel sheet names."""
    sheet_names = []
    ...
    if data_file.get_file_extension() in ['.xlsx', '.xls', '.xlsm']:
        try:
            ...
            logger.error(f"Error reading Excel sheets for file {data_file.id}: {e}")
            messages.error(request, "Could not read Excel worksheets (encrypted or corrupt?).")
...
def stream_arrow_data(request, data_file, selected_sheet, force_refresh: bool = False):
    ...
    try:
        ...
        if table is None and not force_refresh:
            table = read_cached_table(data_file, selected_sheet)
        ...
        if table is None:
            ...
            try:
                ...
                logger.warning(f"Writing Parquet cache failed (continuing without cache): {ex}")

            # Jetzt Arrow-Table aus df erzeugen (einmalig)
        ...
        logger.error(f"Error processing/converting file {data_file.id} to Arrow: {e}", exc_info=True)
     
  
        logger.info("[ANALYZE] Returning Arrow stream")
        return HttpResponse(f"Error during data processing: {e}", status=500)
...
def load_and_process_file(data_file, selected_sheet=None):
    ...
    elif extension == '.parquet' and pq is not None:
